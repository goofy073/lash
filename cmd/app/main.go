...
func main() {
	...
	channelManager := channel.NewManager(b)
	paymentService := payment.NewPaymentService(tm, purchaseRepository, channelManager, customerRepository, b, cryptoPayClient, yookasaClient, referralRepository, cache)
	...
	syncService := sync.NewSyncService(channelManager, customerRepository)

	h := handler.NewHandler(syncService, paymentService, tm, customerRepository, purchaseRepository, cryptoPayClient, yookasaClient, referralRepository, cache, channelManager)
...
func setupSubscriptionNotifier(subService *notification.ChannelAccessService) *cron.Cron {
	c := cron.New()
