...
func (s PaymentService) ProcessPurchaseById(ctx context.Context, purchaseId int64) error {
	...
	// Calculate expiration date based on purchased months
	expireAt := time.Now().UTC().AddDate(0, purchase.Month, 0)
	
	// Create channel invite link for the customer
	inviteLink, err := s.channelManager.GenerateInviteLink(ctx, customer.TelegramID, expireAt)
	if err != nil {
		return err
	}

	// Unban user if they were previously banned
	err = s.channelManager.UnbanUser(ctx, customer.TelegramID)
	if err != nil {
		slog.Warn("Failed to unban user (may not have been banned)", "telegramId", customer.TelegramID, "error", err)
	}
	...
	// Grant referral bonus as additional channel access days
	refereeExpireAt := time.Now().UTC().AddDate(0, 0, config.GetReferralDays())
	refereeInviteLink, err := s.channelManager.GenerateInviteLink(ctxReferee, refereeCustomer.TelegramID, refereeExpireAt)
	if err != nil {
		return err
	}
	refereeUserFilesToUpdate := map[string]interface{}{
		"channel_invite_link": refereeInviteLink,
		"expire_at":           &refereeExpireAt,
		"is_in_channel":       true,
	}
...
func (s PaymentService) ActivateTrial(ctx context.Context, telegramId int64) (string, error) {
	...
	// Create trial channel invite link
	inviteLink, err := s.channelManager.CreateTrialInviteLink(ctx, telegramId, config.TrialDays())
	if err != nil {
		slog.Error("Error creating trial invite link", err)
		return "", err
	}

	// Calculate trial expiration date
	trialExpireAt := time.Now().UTC().AddDate(0, 0, config.TrialDays())

	customerFilesToUpdate := map[string]interface{}{
		"channel_invite_link": inviteLink,
		"expire_at":           &trialExpireAt,
		"is_in_channel":       false, // Will be true once they join
	}
