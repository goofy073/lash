...
type Customer struct {
	...
	ChannelInviteLink *string    `db:"channel_invite_link"`
	Language          string     `db:"language"`
	IsInChannel       bool       `db:"is_in_channel"`
}

func (cr *CustomerRepository) FindByExpirationRange(ctx context.Context, startDate, endDate time.Time) (*[]Customer, error) {
	buildSelect := sq.Select("id", "telegram_id", "expire_at", "created_at", "channel_invite_link", "language", "is_in_channel").
		From("customer").
		Where(
			sq.And{
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&customer.ExpireAt,
			&customer.CreatedAt,
			&customer.ChannelInviteLink,
			&customer.Language,
			&customer.IsInChannel,
		)
		if err != nil {
...
func (cr *CustomerRepository) CreateBatch(ctx context.Context, customers []Customer) error {
	...
	builder := sq.Insert("customer").
		Columns("telegram_id", "expire_at", "language", "channel_invite_link", "is_in_channel").
		PlaceholderFormat(sq.Dollar)
	for _, cust := range customers {
		builder = builder.Values(cust.TelegramID, cust.ExpireAt, cust.Language, cust.ChannelInviteLink, cust.IsInChannel)
	}
...
func (cr *CustomerRepository) UpdateBatch(ctx context.Context, customers []Customer) error {
	...
	query := "UPDATE customer SET expire_at = c.expire_at, language = c.language, channel_invite_link = c.channel_invite_link, is_in_channel = c.is_in_channel FROM (VALUES "
	var args []interface{}
	for i, cust := range customers {
		if i > 0 {
			query += ", "
		}
		query += fmt.Sprintf("($%d::bigint, $%d::timestamp, $%d::text, $%d::text, $%d::boolean)", i*5+1, i*5+2, i*5+3, i*5+4, i*5+5)
		args = append(args, cust.TelegramID, cust.ExpireAt, cust.Language, cust.ChannelInviteLink, cust.IsInChannel)
	}
	query += ") AS c(telegram_id, expire_at, language, channel_invite_link, is_in_channel) WHERE customer.telegram_id = c.telegram_id"
